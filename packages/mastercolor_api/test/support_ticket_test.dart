import 'package:test/test.dart';
import 'package:mastercolor_api/mastercolor_api.dart';

// tests for SupportTicket
void main() {
  final SupportTicket? instance = /* SupportTicket(...) */ null;
  // TODO add properties to the entity

  group(SupportTicket, () {
    // int id
    test('to test the property `id`', () async {
      // TODO
    });

    // String code
    test('to test the property `code`', () async {
      // TODO
    });

    // int clientId
    test('to test the property `clientId`', () async {
      // TODO
    });

    // int soldUnitId
    test('to test the property `soldUnitId`', () async {
      // TODO
    });

    // int productId
    test('to test the property `productId`', () async {
      // TODO
    });

    // TicketCategory category
    test('to test the property `category`', () async {
      // TODO
    });

    // TicketPriority priority
    test('to test the property `priority`', () async {
      // TODO
    });

    // String subject
    test('to test the property `subject`', () async {
      // TODO
    });

    // String description
    test('to test the property `description`', () async {
      // TODO
    });

    // TicketStatus status
    test('to test the property `status`', () async {
      // TODO
    });

    // TicketChannel channel
    test('to test the property `channel`', () async {
      // TODO
    });

    // int assignedUserId
    test('to test the property `assignedUserId`', () async {
      // TODO
    });

    // String assignedUserName
    test('to test the property `assignedUserName`', () async {
      // TODO
    });

    // bool isWarrantyCovered
    test('to test the property `isWarrantyCovered`', () async {
      // TODO
    });

    // DateTime slaDueAt
    test('to test the property `slaDueAt`', () async {
      // TODO
    });

    // DateTime firstResponseAt
    test('to test the property `firstResponseAt`', () async {
      // TODO
    });

    // DateTime resolvedAt
    test('to test the property `resolvedAt`', () async {
      // TODO
    });

    // DateTime closedAt
    test('to test the property `closedAt`', () async {
      // TODO
    });

    // int rating
    test('to test the property `rating`', () async {
      // TODO
    });

    // DateTime createdAt
    test('to test the property `createdAt`', () async {
      // TODO
    });

    // DateTime updatedAt
    test('to test the property `updatedAt`', () async {
      // TODO
    });

  });
}
