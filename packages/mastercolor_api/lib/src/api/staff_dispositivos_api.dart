...
class StaffDispositivosApi {

  final Dio _dio;

  const StaffDispositivosApi(this._dio);
  ...
  Future<Response<ApiSuccess>> supportDevicesPost({ 
...
_responseData = rawData == null ? null : deserialize<ApiSuccess, ApiSuccess>(rawData, 'ApiSuccess', growable: true);
  ...
  /// Returns a [Future] containing a [Response] with a [ApiSuccess] as data
  /// Throws [DioException] if API call or serialization fails
  Future<Response<ApiSuccess>> supportDevicesTokenDelete({ 
    ...
    final _path = r'/support/devices/{token}'.replaceAll('{' r'token' '}', token.toString());
    final _options = Options(
      method: r'DELETE',
      headers: <String, dynamic>{
        ...?headers,
