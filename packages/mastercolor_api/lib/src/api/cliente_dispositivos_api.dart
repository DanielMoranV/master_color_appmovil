...
class ClienteDispositivosApi {

  final Dio _dio;

  const ClienteDispositivosApi(this._dio);
  ...
  Future<Response<ApiSuccess>> clientDevicesPost({ 
    ...
    try {
      _bodyData = jsonEncode(deviceTokenRequest);
...
_responseData = rawData == null ? null : deserialize<ApiSuccess, ApiSuccess>(rawData, 'ApiSuccess', growable: true);
  ...
  /// Returns a [Future] containing a [Response] with a [ApiSuccess] as data
  /// Throws [DioException] if API call or serialization fails
  Future<Response<ApiSuccess>> clientDevicesTokenDelete({ 
    ...
    final _path = r'/client/devices/{token}'.replaceAll('{' r'token' '}', token.toString());
    final _options = Options(
      method: r'DELETE',
      headers: <String, dynamic>{
        ...?headers,
